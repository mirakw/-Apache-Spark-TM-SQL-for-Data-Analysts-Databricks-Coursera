...
SELECT 
quantity * unitPrice AS totalAmount, stockCode, quantity, unitPrice, Country
FROM 
  outdoorProducts
WHERE 
  quantity > 0;
...
SELECT
  SUM(quantity) AS totalQuantity, Country
FROM
  sales
GROUP BY
  Country
...
FROM countryCodes 
JOIN salesQuants ON EnglishShortName = Country;
